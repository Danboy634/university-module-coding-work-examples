...
EnemyTurtleWorm::EnemyTurtleWorm()
{
}
...
void EnemyTurtleWorm::Initialise()
{
	m_Health = 35;
	m_PointsForDeath = 35;
	...
	SpritesheetDef* pSpritesheetDef = pGameDataManager->GetSpritesheetDef("sprites/characters/spr_wormTurtle.xml");

	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Idle, pSpritesheetDef, "spr_wormTurtle_idle", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Walk, pSpritesheetDef, "spr_wormTurtle_walk", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Precision, pSpritesheetDef, "spr_wormTurtle_death", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Death, pSpritesheetDef, "spr_wormTurtle_death", 10, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Hit, pSpritesheetDef, "spr_wormTurtle_hit", 10, false);
	m_AnimatedSprite.SetCurrentAnimation(E_AnimationState_Idle);

	EnemyBase::Initialise();
}

void EnemyTurtleWorm::toIdleState()
{
...
void EnemyTurtleWorm::toWalkingState()
{
...
void EnemyTurtleWorm::toAttackingState()
{
	...
	pAudioManager->PlaySoundFX("sounds/sfx_pistol.wav"); 
	m_AnimatedSprite.SetCurrentAnimation(E_AnimationState_Walk);
}

void EnemyTurtleWorm::Update()
{
	if (!m_EnemyDead)
	{
...
void EnemyTurtleWorm::UpdateWeapon()
{
	if (m_pWeapon)
	{
...
void EnemyTurtleWorm::GetPosition(int& x, int& y)
{
	EnemyBase::GetPosition(x, y);
}

void EnemyTurtleWorm::OnCollision(Collider2D * pCollider2D)
{
