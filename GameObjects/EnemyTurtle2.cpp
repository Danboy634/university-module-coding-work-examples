...
void EnemyTurtle2::Initialise()
{
	...
	m_pWeapon = new ShotgunWeapon();
	m_pWeapon->Initialise();

	GameDataManager* pGameDataManager = C_SysContext::Get <GameDataManager>();
	SpritesheetDef* pSpritesheetDef = pGameDataManager->GetSpritesheetDef("sprites/characters/spr_turtle2.xml");

	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Idle, pSpritesheetDef, "spr_turtle2_idle", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Walk, pSpritesheetDef, "spr_turtle2_walk", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Precision, pSpritesheetDef, "spr_turtle2_precision", 10, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Death, pSpritesheetDef, "spr_turtle2_death1", 10, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Hit, pSpritesheetDef, "spr_turtle2_hit", 10, false);
	m_AnimatedSprite.SetCurrentAnimation(E_AnimationState_Idle);

	EnemyBase::Initialise();
}

void EnemyTurtle2::toIdleState()
{
...
void EnemyTurtle2::toWalkingState()
{
...
void EnemyTurtle2::toAttackingState()
{
...
void EnemyTurtle2::Update()
{
	if (!m_EnemyDead)
	{
		...
		else
		{
			...
			if (m_CurrentState != E_EnemyState_ATTACKING || distance > 280)
			{
...
void EnemyTurtle2::UpdateWeapon()
{
	if (m_pWeapon)
	{
...
void EnemyTurtle2::GetPosition(int& x, int& y)
{
	EnemyBase::GetPosition(x, y);
}

void EnemyTurtle2::OnCollision(Collider2D * pCollider2D)
{
