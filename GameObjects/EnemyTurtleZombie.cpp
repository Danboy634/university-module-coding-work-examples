...
EnemyTurtleZombie::EnemyTurtleZombie()
{
}
...
void EnemyTurtleZombie::Initialise()
{
	m_Health = 35;
	m_PointsForDeath = 12;
	...
	float randTargetX = (float)(1000 + (rand() % (int)975));
	float randTargetY = (float)(1000 + (rand() % (int)925));
	...
	SpritesheetDef* pSpritesheetDef = pGameDataManager->GetSpritesheetDef("sprites/characters/spr_zombieTurtle.xml");

	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Idle, pSpritesheetDef, "spr_zombieTurtle_idle", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Walk, pSpritesheetDef, "spr_zombieTurtle_walk", 10, true);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Precision, pSpritesheetDef, "spr_zombieTurtle_precision", 10, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Death, pSpritesheetDef, "spr_zombieTurtle_death", 9, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Hit, pSpritesheetDef, "spr_zombieTurtle_hit", 10, false);
	m_AnimatedSprite.AddAnimationFromSpriteSheetDef(E_AnimationState_Rise, pSpritesheetDef, "spr_zombieTurtle_rise", 7, false);
	m_AnimatedSprite.SetCurrentAnimation(E_AnimationState_Idle);

	EnemyBase::Initialise();
}

void EnemyTurtleZombie::toIdleState()
{
...
void EnemyTurtleZombie::toWalkingState()
{
...
void EnemyTurtleZombie::toAttackingState()
{
...
void EnemyTurtleZombie::Update()
{
	if (!m_EnemyDead)
	{
		...
		else
		{
			...
			if (m_CurrentState != E_EnemyState_ATTACKING || distance > 350)
			{
...
void EnemyTurtleZombie::UpdateWeapon()
{
	if (m_pWeapon)
	{
...
void EnemyTurtleZombie::GetPosition(int& x, int& y)
{
	EnemyBase::GetPosition(x, y);
}

void EnemyTurtleZombie::OnCollision(Collider2D * pCollider2D)
{
	if (pCollider2D->GetCollisionType() == E_Collider_Bullet_Player && m_AnimatedSprite.GetCurrentAnimation() != E_AnimationState_Death)
	{
		...
		particleEffectHit->Initialise(0, "sprites/effects/spr_hit_light.xml", "spr_hit_light", 10, false, (int)m_AnimatedSprite.getPosition().x, (int)m_AnimatedSprite.getPosition().y);
